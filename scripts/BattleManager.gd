...
func find_new_targets_for_orphaned_cards(allies: Array, enemies: Array):
	...
	# Выполняем бой с новыми назначениями
	if new_assignments.enemy_to_allies.size() > 0 or new_assignments.ally_to_enemies.size() > 0:
